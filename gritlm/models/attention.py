...
def my_dot_product_attention_weights(
  ...
  assert len(query.shape) == len(key.shape), 'q, k must have same rank.'
  ...
  if apply_tanh:
    attn_weights = (30*torch.tanh(attn_weights/30)).type(dtype)
  if bias is not None:
  ...
  if not deterministic and dropout_rate > 0.0:
    ...
    if broadcast_dropout:
      ...
      dropout_shape = tuple([1] * (len(key.shape) - 2)) + attn_weights.shape[-2:]
      keep = torch.bernoulli(torch.full(dropout_shape, keep_prob))  # type: ignore
    else:
      keep = torch.bernoulli(torch.full(attn_weights.shape, keep_prob))  # type: ignore
    multiplier = keep.type(dtype) / torch.tensor(keep_prob, dtype=dtype)
